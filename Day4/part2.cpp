...
int main() {
	...
	// then check if there is any overlap at all
	...
	while (getline(infile, line)) {
		...
		if (startInt1 <= endInt2 && endInt1 >= startInt2) {//then check if the smaller range is within the larger range.
			++rangePairCount;
		}
	}
	
	std::cout << rangePairCount << std::endl;
